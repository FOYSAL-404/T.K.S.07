...
def banner():
 ...
 ________    ___   ____  ____   ______        _       _____     
|_   __  | .'   `.|_  _||_  _|.' ____ \      / \     |_   _|    
  | |_ \_|/  .-.  \ \ \  / /  | (___ \_|    / _ \      | |      
  |  _|   | |   | |  \ \/ /    _.____`.    / ___ \     | |   _  
 _| |_    \  `-'  /  _|  |_   | \____) | _/ /   \ \_  _| |__/ | 
|_____|    `.___.'  |______|   \______.'|____| |____||________| 
                                                                

                                  
what's App: +8801793268786                                 
...
def login_lagi334():
	try:
		...
		cookie=input(f'  [тАв] INPUT COOKIES :{asu} ')
...
def result():
	...
	if kz in ['1','01']:
		try:vin = os.listdir('CP')
			...
			back()
		if len(vin)==0:
			print('=>Anda Tidak Memiliki FOYSAL CP ')
			time.sleep(2)
			back()
		else:
			cih = 0
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('=>ENTER LIMT IDZ [MAX:10] : '))
	except ValueError:
		print('{k}=>NOT JANI ')
		exit()
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	cetak(nel('[bold cyan]               WAIT FOR CRACKING[bold cyan]')) 
	print(f'                    {m}A {k}K {h}A {u}S {b}H {u}    {b}K {u}U {h} H{k} T{m} I{b} A{h}')
	print('')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,idf,pwv)
	print('')
	cetak(nel('\t[purple]=>[green] Cracking End [purple] <<[yellow] '))
	print(f'[{h}FOYSAL{u}]{h} OK : {h}%s '%(ok))
	print(f'{k}[{k}FOYSAL{h}]{k} CP : {k}%s{u} '%(cp))
	print('')
	print('=>Do You Continue ( Y/t ) ? ')
	woi = input('=>Choose  : ')
	if woi in ['y','Y']:
		back()
	else:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}FOYSAL>{P}[{k}{loop}{P}/{h}{len(id)}{P}]OK{P}[{H}{ok}{P}]CK{P}[{k}{cp}{x}]KST[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
